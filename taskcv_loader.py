    ...
    def __next__(self):
        ...
        if self.stop_S and self.stop_T or self.iter > self.max_dataset_size:
            ...
            if self.flip and random.random() < 0.5:
                ...
                idx = paddle.to_tensor(idx, 'float64')
                S = S.index_select(3, idx)
                T = T.index_select(3, idx)
            return {'S': S, 'S_label': S_paths, 'S_index': S_indexes, 'T':
                T, 'T_label': T_paths, 'T_index': T_indexes}


class CVDataLoader(object):

    def initialize(self, dataset_S, dataset_T, batch_size, shuffle=True,
        drop_last=False):
        self.max_dataset_size = float('inf')
        data_loader_S = torch2paddle.DataLoader(dataset_S, batch_size=\
            batch_size, shuffle=shuffle, drop_last=drop_last, num_workers=0)
        data_loader_T = torch2paddle.DataLoader(dataset_T, batch_size=\
            batch_size, shuffle=shuffle, drop_last=drop_last, num_workers=0)
        self.dataset_S = dataset_S
        self.dataset_T = dataset_T
        flip = False
        self.paired_data = PairedData(data_loader_S, data_loader_T, self.
            max_dataset_size, flip)
    ...
    def __len__(self):
        return min(max(len(self.dataset_S), len(self.dataset_T)), self.
            max_dataset_size)
