...
def extract_archive(from_path, to_path=None, remove_finished=False):
    ...
    if _is_tar(from_path):
        ...
        to_path = os.path.join(to_path, os.path.splitext(os.path.basename(
            from_path))[0])
...
def download_and_extract_archive(url, download_root, extract_root=None,
    filename=None, md5=None, remove_finished=False):
...
def verify_str_arg(value, arg=None, valid_values=None, custom_msg=None):
    ...
    if value not in valid_values:
        if custom_msg is not None:
            ...
            msg = msg.format(value=value, arg=arg, valid_values=\
                iterable_to_str(valid_values))
        raise ValueError(msg)
    return value
